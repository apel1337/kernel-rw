...
namespace mem {
	...
	bool find_driver() {
		driver_handle = CreateFileW(L"\\\\.\\{4d36e97d-e325-11ce-bfc1-08002be10318}", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
template <typename T>
T write(uintptr_t address, T buffer) {
	mem::write_physical((PVOID)address, &buffer, sizeof(T));
	return buffer;
}
bool is_valid(const uintptr_t adress)
{
